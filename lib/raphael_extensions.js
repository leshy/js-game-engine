...
Raphael.fn.arrow = function (attr) {
    ...
    function arrow(attr) {
        this.attr = _.extend({ x1: 0, y1: 0, x2: 100, y2: 100, size: 3, color: "red"}, attr)
        this.render()
    }
    ...
    arrow.prototype.render = function() {
        ...
        this.arrowPath = paper.path("M" + x2 + " " + y2 + " L" + (x2 - size) + " " + (y2 - size) + " L" + (x2 - size) + " " + (y2 + size) + " L" + x2 + " " + y2 ).attr({"fill":this.attr.color, "stroke": this.attr.color }).rotate((90+angle),x2,y2);
        this.linePath = paper.path("M" + x1 + " " + y1 + " L" + x2 + " " + y2).attr({"stroke":this.attr.color});
    }
